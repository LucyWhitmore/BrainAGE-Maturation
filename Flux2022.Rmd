...
t.test(sample_1$interview_age, sample_2$interview_age)

#Save samples
save(brainage_pds, file="BrainagePDS.Rda")
save(sample_1, file="Sample1.Rda")
save(sample_2, file = "Sample2.Rda")
...
summary(baseline_cognition_fluid_1)
summary(baseline_cognition_fluid_2)

summary(baseline_cognition_crystal_1)
summary(baseline_cognition_crystal_2)
...
ggplot(sample_1, aes(x = youth_mean, y = corrected_gap)) +
  stat_smooth(method = "lm", col = "#F08000", fill="#F08000") +
  xlab("Pubertal Development (Youth-Report)") +
  ylab("BrainAGE") +
  ggtitle("Predicted Values") +
  theme_minimal()

# Parent-Report Puberty
ggplot(sample_1, aes(x = parent_mean, y = corrected_gap)) +
    stat_smooth(method = "lm", col = "#F08000", fill="#F08000") +
  xlab("Pubertal Development (Parent-Report)") +
  ylab("BrainAGE") +
  ggtitle("Predicted Values") +
  theme_minimal()

#Cognition
ggplot(sample_1, aes(x = nihtbx_totalcomp_agecorrected, y = corrected_gap)) +
    stat_smooth(method = "lm", col = "#F08000", fill="#F08000") +
  xlab("Age-Corrected Cognition Scores") +
  ylab("BrainAGE") +
  ggtitle("Predicted Values") +
  theme_minimal()
...
#Old Versions
plot_model(baseline_puberty_p_mean_1, type="pred", terms = "parent_mean", title = "Predicted Gap") +
  theme_sjplot()


plot_model(baseline_cognition_1, type="pred", terms = "nihtbx_totalcomp_agecorrected", title = "Predicted Gap")+
  theme_sjplot()

plot_model(baseline_cognition_2, type="pred", terms = "nihtbx_totalcomp_agecorrected")
