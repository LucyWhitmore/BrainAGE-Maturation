...
vip_abcd_baseline <- vip(fit_workflow_combat, num_features=20) +
  theme_light()
...
vip_abcd_followup <- vip(fit_workflow_combat_followup, num_features=20) +
  theme_light()
...
abcd_vi_dk <- left_join(vi_updated, abcd_vi_renamed_dk)
...
abcd_model_dk_vol_importance_baseline <- ggplot(dk_vi_vol) +
...
abcd_model_dk_area_importance_baseline <- ggplot(dk_vi_area) +
  geom_brain(atlas = dk, position = position_brain(hemi ~ side), aes(fill = Importance))+
  theme_void() +
  scale_fill_gradient(low = '#FDD872', high = '#E76F51') +
...
abcd_model_aseg_importance_coronal_baseline <- ggplot(aseg_vi) +
...
abcd_model_aseg_importance_sagittal_baseline <- ggplot(aseg_vi) +
...
abcd_model_aseg_importance_baseline <- ggarrange(abcd_model_aseg_importance_coronal_baseline, abcd_model_aseg_importance_sagittal_baseline, 
          ncol = 1, nrow = 2)


# Save as final figure
ggarrange(vip_abcd_baseline, abcd_model_dk_vol_importance_baseline, abcd_model_dk_area_importance_baseline, abcd_model_aseg_importance_baseline, 
          labels = c("A", "B", "C", "D"),
          ncol = 2, nrow = 2)
ggsave(filename = "mod2_vip_ggseg.tiff",
                          width=12, height=12, units='in', dpi=300)

```

Optional feature name troubleshooting for ggseg
```{r}
...
ggsave(filename = "pred_bias_mod1_combat.tiff",
                          width=10, height=10, units='in', dpi=300)
...
ggsave(filename = "pred_bias_mod2_combat.tiff",
                          width=10, height=10, units='in', dpi=300)
...
### Rerun old figures
# Distributions - Baseline (Model 1)
```{r}
...
ggsave(filename = "distributions_baseline_mod1_combat.tiff",
                          width=7, height=5, units='in', dpi=300)

```

# Distributions - Follow-Up (Model 1)
```{r}
#Youth-Report Puberty
#make categorical variable for puberty
#age-corrected cognition: These are presented as Standard Scores (mean=100, SD=15).
...
ggsave(filename = "distributions_followup_mod1_combat.tiff",
                          width=10, height=4, units='in', dpi=300)

```

# Distributions - Baseline (Model 2)
```{r}
#Youth-Report Puberty
#make categorical variable for puberty
#age-corrected cognition: These are presented as Standard Scores (mean=100, SD=15).
...
ggsave(filename = "distributions_baseline_mod2_combat.tiff",
                          width=7, height=5, units='in', dpi=300)


```

# Distributions - Follow-Up (Model 2)
```{r}
#Youth-Report Puberty
#make categorical variable for puberty
#age-corrected cognition: These are presented as Standard Scores (mean=100, SD=15).
...
ggsave(filename = "distributions_followup_mod2_combat.tiff",
                          width=10, height=4, units='in', dpi=300)

```

# Model features - follow-up
```{r}
...
abcd_model_dk_vol_importance_followup <- ggplot(dk_vi_vol_followup) +
  geom_brain(atlas = dk, position = position_brain(hemi ~ side), aes(fill = Importance))+
  theme_void() +
  scale_fill_gradient(low = '#FDD872', high = '#E76F51') +
  labs(title = "Cortical Volume")
...
abcd_model_dk_area_importance_followup <-ggplot(dk_vi_area_followup) +
  geom_brain(atlas = dk, position = position_brain(hemi ~ side), aes(fill = Importance))+
  theme_void() +
  scale_fill_gradient(low = '#FDD872', high = '#E76F51') +
  labs(title = "Cortical Area")
...
abcd_model_aseg_importance_coronal_followup <- ggplot(aseg_vi_followup) +
  geom_brain(atlas = aseg, side="coronal", aes(fill = Importance))+
  theme_void() +
  scale_fill_gradient(low = '#FDD872', high = '#E76F51') +
  labs(title = "Subcortical Volume")
...
abcd_model_aseg_importance_sagittal_followup <- ggplot(aseg_vi_followup) +
  geom_brain(atlas = aseg, side="sagittal", aes(fill = Importance))+
  theme_void() +
  scale_fill_gradient(low = '#FDD872', high = '#E76F51')
...
# make one aseg figure
abcd_model_aseg_importance_followup <- ggarrange(abcd_model_aseg_importance_coronal_followup, abcd_model_aseg_importance_sagittal_followup, 
          ncol = 1, nrow = 2)


# Save as final figure
ggarrange(vip_abcd_followup, abcd_model_dk_vol_importance_followup, abcd_model_dk_area_importance_followup, abcd_model_aseg_importance_followup, 
          labels = c("A", "B", "C", "D"),
          ncol = 2, nrow = 2)
ggsave(filename = "mod2_vip_ggseg_followup.tiff",
                          width=12, height=12, units='in', dpi=300)
...
Check how many baseline training v analysis are in follow-up
```{r}
...
mod1_longitudinal <- ggplot(data = brainage_both_waves_long %>% filter(src_subject_id %in% brainage_slope_count$src_subject_id) %>% 
...
mod2_longitudinal <- ggplot(data = brainage_abcd_both_waves_long %>% filter(src_subject_id %in% brainage_abcd_slope_count$src_subject_id) %>% 
                              group_by(src_subject_id) %>%
                mutate(slope = diff(corrected_gap)),
       aes(x = truth, y = corrected_gap, group = src_subject_id, color = slope > 0)) +
    geom_line(size = .2, alpha=0.4) +
...
# Save as final figure
ggarrange(mod1_longitudinal, mod2_longitudinal, 
          labels = c("A", "B"),
          ncol = 2, nrow = 1)
ggsave(filename = "longitudinal_plot.tiff",
