...
cognition <- rio::import("/Volumes/devbrainlab/ABCD_Data/ABCD4pt0/abcd_tbss01.txt") %>%
  filter(!collection_title=="collection_title")  %>% 
  select(src_subject_id,interview_age, eventname, nihtbx_fluidcomp_agecorrected, nihtbx_cryst_agecorrected, nihtbx_totalcomp_agecorrected, nihtbx_totalcomp_rawscore, nihtbx_totalcomp_cs, nihtbx_totalcomp_itmcnt) %>% 
  mutate_at(c(2, 4:9), as.numeric)
...
social <- rio::import("/Volumes/devbrainlab/ABCD_Data/ABCD4pt0/abcd_gish2y01.txt") %>%
  filter(!collection_title=="collection_title")  %>% 
  select(-c(subjectkey, interview_date, interview_age, gish2_device)) %>% 
